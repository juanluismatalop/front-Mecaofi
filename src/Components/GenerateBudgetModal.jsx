...
// Función auxiliar para calcular el precio final (mantiene 2 decimales para precio final)
const calcularPrecioFinal = (precio, descuentoPorcentaje) => {
    ...
    // Redondeamos el precio final a 2 decimales (porque es una cantidad de dinero/moneda)
    return precioFinal.toFixed(2);
};

// =========================================================================
// 🚨 SUBCOMPONENTE: MachineBudgetItem (Actualizado el step para costes)
// =========================================================================
const MachineBudgetItem = ({ maquina, onUpdate, onRemove, saving }) => {
    ...
    const handleFieldChange = (field, value) => {
        ...
        // 🚨 Para Costes: Si el usuario introduce un valor con más de 3 decimales, lo forzamos a 3 decimales 
        // para mantener la consistencia en el estado, aunque el campo solo permite 3.
        if (field === 'costePorCopiaColor' || field === 'costePorCopiaNegro') {
            const numValue = parseFloat(value);
            // Convertimos a string con 3 decimales solo si el valor es numérico y se ha ingresado algo.
            const formattedValue = isNaN(numValue) || value === '' ? value : numValue.toFixed(3);
            onUpdate(maquina.Id, field, formattedValue);
        }
    };
                ...
                <div className="machine-budget-fields">
                    ...
                    <div className="form-row">
                        <div className="form-group">
                            ...
                            <input
                                type="number"
                                // 🚨 CAMBIO A 0.001 PARA 3 DECIMALES
                                step="0.001" 
                                min="0"
...
export default function GenerateBudgetModal({ isOpen, onClose, currentIdCliente, clientName, visitaId = null }) {
    ...
    const [impuestoRenting, setImpuestoRenting] = useState(''); // Impuesto/Interés del Renting (€ o %)
    ...
    const handleAddMaquina = () => {
        ...
        if (maquina) {
            setMaquinasSeleccionadas(prev => [...prev, {
                ...
                // 🚨 Inicializamos los costes a 3 decimales si tienen valor (o vacío si no)
                costePorCopiaColor: maquina.CostePorCopiaColor !== null && maquina.CostePorCopiaColor !== undefined 
                    ? parseFloat(maquina.CostePorCopiaColor).toFixed(3) : '', 
                costePorCopiaNegro: maquina.CostePorCopiaNegro !== null && maquina.CostePorCopiaNegro !== undefined 
                    ? parseFloat(maquina.CostePorCopiaNegro).toFixed(3) : '', 
            }]);
            setSelectedMaquina('');
            setError(null);
        }
    ...
    const validarFormulario = () => {
        ...
        for (const maquina of maquinasSeleccionadas) {
            ...
            if (!maquina.costePorCopiaNegro || parseFloat(maquina.costePorCopiaNegro) < 0 || isNaN(parseFloat(maquina.costePorCopiaNegro))) {
        ...
        if (tipoRenting === 'Meses' || tipoRenting === '30/60/90') {
            if (!impuestoRenting || parseFloat(impuestoRenting) <= 0 || isNaN(parseFloat(impuestoRenting))) {
                setError('El campo de Precio por mes o Interés de Renting debe ser un valor numérico positivo.');
                return false;
            }
        }
        ...
        if (tipoRenting === 'Meses') {
            if (!mesesRenting || parseInt(mesesRenting, 10) <= 0 || isNaN(parseInt(mesesRenting, 10))) {
                setError('Para la opción "Renting", debe especificar una cantidad de meses entera y positiva.');
                return false;
            }
        }
    ...
    // 🚨 MÉTODO PARA GENERAR PDF y DESCARGAR (Asegura el parseo a float de 3 decimales para costes)
            ...
            const pdfData = {
                ...
                maquinas: maquinasSeleccionadas.map(maquina => {
                    ...
                    return ({
                        ...
                        // 🚨 Usamos parseFloat para enviar la cantidad exacta (hasta 3 decimales)
    ...
    // 🚨 MÉTODO DE ENVÍO PRINCIPAL (Asegura el parseo a float de 3 decimales para costes)
                ...
                <div className="modal-body">
                    ...
                    <div className="section-renting-and-observations">
                        <div className="renting-controls">
                            ...
                            {(tipoRenting === 'Meses' || tipoRenting === '30/60/90') && (
                                <div className="form-group" style={{ maxWidth: '150px', marginBottom: '15px' }}>
                                    <label htmlFor="impuestoRenting">
                                        {tipoRenting === 'Meses' ? 'Precio por mes (€)' : 'Interés (%)'}
                                    </label>
                                    <input
                                        id="impuestoRenting"
                                        type="number"
                                        ...
                                        placeholder={tipoRenting === 'Meses' ? 'Ej: 50.00' : 'Ej: 5'}
                                        disabled={saving}
                                        required
                                    />
                                </div>
